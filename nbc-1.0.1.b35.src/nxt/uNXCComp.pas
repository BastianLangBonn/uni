...
uses
  SysUtils, Math, uNXCLexer, uNBCLexer, mwGenericLex,
...
resourcestring
  ...
  sUnsupportedArrayExpr    = 'Unsupported array expression';
...
const                                     // 'xileweRWve'
  KWcode: array[1..NKW1+1] of Char =
    (TOK_IDENTIFIER, TOK_IF, TOK_ELSE, TOK_WHILE,
     ...
     TOK_CHARDEF, TOK_BYTEDEF, TOK_BYTEDEF, TOK_MUTEXDEF, TOK_STRINGDEF,
     TOK_END,
     #0);
...
begin
  if (Token <> TOK_STRINGLIT) and
     (DataType(Value) <> TOK_STRINGDEF) then
        ...
        TOK_IDENTIFIER : begin
          ...
          begin
            ...
            if IsArrayType(fLHSDataType) or IsUDT(fLHSDataType) then
              EmitLn(Format('index %s, %s, %s',[GetDecoratedIdent(fLHSName), GetDecoratedIdent(savedvalue), RegisterName]))
            else
              EmitLn(Format('index %0:s, %1:s, %0:s',[RegisterName, GetDecoratedIdent(savedvalue)]));
          end
...
//            if GetArrayDimension(fLHSDataType) <> GetArrayDimension(rdt) then
...
begin
  ...
  begin
    val := Value; // must be a single dimensional array of string
    if DataType(val) <> TOK_ARRAYSTRING then
  ...
  begin
    ...
    begin
      ...
      if IsArrayType(dt) then
        AbortMsg(sUnsupportedArrayExpr)
      else
        NumericRelation;
    end
    else
...
begin
  ...
  begin
    ...
    case Token of
      ...
      TOK_MUTEXDEF, TOK_STRINGDEF : begin
        tname := Value;
        vt := Token;
...
begin
  ...
  while (Token in [TOK_DIRECTIVE, TOK_UNSIGNED, TOK_CONST,
    ...
    TOK_BYTEDEF, TOK_MUTEXDEF, TOK_STRINGDEF]) and not endofallsource do
  begin
    case Token of
      TOK_DIRECTIVE : begin
        ProcessDirectives;
...
begin
  ...
  begin
    ...
    if Token in [TOK_CHARDEF, TOK_BYTEDEF, TOK_SHORTDEF, TOK_LONGDEF,
      TOK_MUTEXDEF, TOK_STRINGDEF, TOK_USERDEFINEDTYPE, TOK_STRINGLIT] then
    begin
      if protoexists then
      begin
...
//        GS_ParamType[procpos, protocount] := ptype;
    ...
    while (Token = TOK_COMMA) and not endofallsource do begin
      ...
      begin
        ...
        if Token in [TOK_CHARDEF, TOK_BYTEDEF, TOK_SHORTDEF, TOK_LONGDEF,
                     TOK_MUTEXDEF, TOK_STRINGDEF, TOK_USERDEFINEDTYPE] then
        begin
          if protoexists then
          begin
            Expected(sParameterList);
...
begin
  ...
  begin
    ...
    if bFuncStyle then
      Next;
    EmitLn(Format('mov __result_%s, %s',[fCurrentThreadName, Value]));
    if bFuncStyle then
      Next;
  end
  else if dt <> #0 then
...
procedure TNXCComp.ProcessStruct;
var
...
begin
  ...
  begin
    ...
    if dt = dsCluster then
      EmitLn(Format('%s %s%s', [mname, mtype, aval]))
    else
      EmitLn(Format('%s %s%s', [mname, TypeToStr(dt), aval]));
...
begin
  ...
  begin
    V := fGlobals[i];
    if IsArrayType(V.DataType) and (V.LenExpr <> '') and (V.Value = '') then
    begin
...
begin
  Result := ((lhs <> TOK_MUTEXDEF) and (rhs <> TOK_MUTEXDEF)) and
            ...
            ((lhs in [TOK_ARRAYBYTEDEF, TOK_STRINGDEF]) and
             (rhs in [TOK_ARRAYBYTEDEF, TOK_STRINGDEF])));
  if not Result then
  begin

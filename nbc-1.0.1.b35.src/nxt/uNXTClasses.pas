...
type
  ...
  TDSType = (dsVoid, dsUByte, dsSByte, dsUWord, dsSWord, dsULong, dsSLong,
    dsArray, dsCluster, dsMutex);

  TSTTFuncType = function(const stype : string; bUseCase : Boolean = false) : TDSType;

const
  BytesPerType : array[TDSType] of Byte = (4, 1, 1, 2, 2, 4, 4, 2, 4, 4);
  NOT_AN_ELEMENT = $FFFF;
...
type
  ...
  public
    ...
    procedure AddValuesFromString(Calc : TNBCExpParser; sargs : string);
...
//  TDataspaceEntry = class;
  ...
  public
    ...
    procedure RemoveReferenceItPresent(const path : string);
...
function GetArgDataType(val : Int64): TDSType;
...
function CreateConstantVar(DSpace : TDataspace; iVal : Int64; bIncCount : boolean) : string;
...
uses
  StrUtils, uNBCLexer, uCommonUtils, uVersionInfo,
...
//  sOutputError        = 'Error: line #: %d %s (%s) (%s)';
//  sOutputWarning      = 'Warning: line #; %d %s (%s) (%s)';
...
procedure TDataspace.RemoveReferenceItPresent(const path: string);
var
  DE : TDataspaceEntry;
begin
...
procedure TDataspaceEntry.AddValuesFromString(Calc : TNBCExpParser; sargs: string);
var
  i : integer;
  SL : TStringList;
begin
  ...
  try
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            AddValue(Cardinal(Trunc(Calc.Value)));
          end;
        end
        else
        begin
          ...
          begin
            ...
            begin
              Calc.Expression := SL[i];
              SubEntries[i].DefaultValue := Cardinal(Trunc(Calc.Value));
            end;
          end;
        end;
      end
      ...
      begin
        ...
        DefaultValue := Cardinal(Trunc(Calc.Value));
      end;
    end;
  finally
    SL.Free;
...
begin
  Result := 0;
  if DataType in [dsVoid..dsSLong, dsMutex] then
  begin
...
begin
  ...
  case StrToOpcode(op, bUseCase) of
    ...
    OPS_DB..OPS_MUTEX : Result := altVarDecl;
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              tmpName := CreateConstantVar(DataSpace, iVal, False);
              // we have a temporary called 'tmpName' in the dataspace
              // so switch our line to use it instead
              arg.Value := tmpName;
            end;
...
begin
  ...
  if tmpabs > MaxInt then
    Result := dsULong
  else if tmpabs > High(SmallInt) then
    Result := dsSLong
  else if tmpabs > High(ShortInt) then
    Result := dsSWord
  else
    Result := dsSByte;
end;
...
begin
  ...
  begin
    dt := GetArgDataType(Trunc(Calc.Value));
...
begin
  ...
  case argType of
    ...
    begin
      CodeSpace.Dataspace.RemoveReferenceItPresent(arg);
    end;
    aatClumpID : begin
...
begin
  ...
  if Self.IsSubroutine then
    CodeSpace.Dataspace.RemoveReferenceItPresent(Format('__%s_return', [Name]));
end;
...
function CreateConstantVar(DSpace : TDataspace; iVal : Int64;
  bIncCount : boolean) : string;
var
  datatype : TDSType;
  DE : TDataspaceEntry;
begin
  datatype := GetArgDataType(iVal);
    ...
    for i := 0 to ClumpCode.Count - 1 do begin
      ...
      case AL.Command of
        OP_SET, OP_MOV : begin
          ...
          // find the next line (which may not be i+1)
          offset := 1;
          while (i < ClumpCode.Count - offset) do begin
            if ClumpCode.Items[i+offset].Command <> OPS_INVALID then
              Break;
            inc(offset);
          end;
              ...
              case ALNext.Command of
                ...
                OP_ADD, OP_SUB, OP_MUL, OP_DIV, OP_MOD, OP_AND, OP_OR, OP_XOR : begin
                  ...
                  if (arg1 = arg2) or (arg1 = arg3) then begin
                    ...
                    // arithop A, B, X
                    if AL.Command = OP_SET then
                      tmp := CreateConstantVar(CodeSpace.Dataspace,
                        StrToIntDef(AL.Args[1].Value, 0), True)
                    else
                      tmp := AL.Args[1].Value;
                    if (arg1 = arg2) then begin

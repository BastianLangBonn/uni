...
float current_value = 0.06;		// Legt den HIGH Anteil vom PWM-Signal fest
					// 0 <= current_value <= 1
int actual_speed = 0;
int lcd_handler;
	...
	while(1){
		...
		if((digitalRead(GPIO_BRAKE1) == 0 && brake_on == 0){
			...
			printf("Bremse gezogen\n");
                ...
                #ifdef BRAKE2
                if((digitalRead(GPIO_BRAKE1) == 0 || GPIO_BRAKE2 == 0 ) && brake_on == 0){
                        brake_on = 1;
			#ifdef DEBUG
			printf("Bremse gezogen\n");
			#endif
		...
		if(digitalRead(GPIO_BRAKE1) == 1 && brake_on == 1){
			...
			printf("Bremse losgelassen\n");
                ...
                #ifdef BRAKE2
		if(digitalRead(GPIO_BRAKE1) == 1 && digitalRead(GPIO_BRAKE2) == 1 brake_on == 1){
			//Bremse wieder losgelassen
			brake_on = 0;
			#ifdef DEBUG
			printf("Bremse losgelassen\n");
...
int main(){
	...
	while(1){
		...
		/*  Ein- und Ausschalten des PWM-Signals  */
                if(brake_on == 0 && motor_on == 1 && motor_limited == 0 && pwm_on = 0){
                        //Einschalten des PWM-Signals
			pwm_on = 1;
                        softPwmWrite(GPIO_PWM, 10);
                        sleep(1);
                        softPwmWrite(GPIO_PWM, (int)(PWM_RANGE * current_value));
                }else if(brake_on == 0 && motor_on == 1 && motor_limited == 0 && pwm_on = 1){
		}else{

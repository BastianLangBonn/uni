...
def trainModel():
    # Read the data
    data = read_csv('./trackData.csv',delimiter = ';')
    ...
    for i in range(len(data)):
        outputs.append([data.energy[i], data.resultingVelocity[i]])
        inputs.append([data.currentVelocity[i], data.passedTime[i], 
                       data.slope[i], data.command[i]])
